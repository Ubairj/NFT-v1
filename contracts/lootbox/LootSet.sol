...
library LootSetLib {
    ...
    function createKey(ILootbox.LootSet storage, ILootbox.Loot memory req) public pure returns(uint256 _hash) {
        _hash = uint256(
            keccak256(abi.encodePacked(
                req.item.collectionId,
                req.item.symbol,
                req.item.name,
     ...
     */
    function insert(ILootbox.LootSet storage self, ILootbox.Loot memory req) public {
        require(
            !exists(self, req),
            "LootSetLib: key already exists in the set."
        );
     ...
     */
    function remove(ILootbox.LootSet storage self, ILootbox.Loot memory key) public {
        require(
            exists(self, key),
            "LootSetLib: key does not exist in the set."
        );
        ...
        if (rowToReplace != last) {
            ...
            ILootbox.Loot memory valueToMove = self.valueList[last];
     ...
     */
    function removeAt(ILootbox.LootSet storage self, uint256 _index) public {
        require(
            _index < count(self),
            "LootSetLib: key does not exist in the set."
        );
     ...
     */
    function count(ILootbox.LootSet storage self) public view returns (uint256 _count) {
     ...
     */
    function exists(ILootbox.LootSet storage self, ILootbox.Loot memory key)
        public
        view
     ...
     */
    function existsKey(ILootbox.LootSet storage self, uint256 keyHash)
        public
        view
        returns (bool _exists)
    {
     ...
     */
    function keyAtIndex(ILootbox.LootSet storage self, uint256 index)
        public
        view
     ...
     */
    function valueAtIndex(ILootbox.LootSet storage self, uint256 index)
        public
        view
        returns (ILootbox.Loot memory _value)
    {
